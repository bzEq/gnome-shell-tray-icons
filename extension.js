...
function createIconsContainer() {
  ...
  // An empty ButtonBox will still display padding, therefore create it without
  // visibility.
...
function onTrayIconAdded(o, icon, role, delay = 1000) {
  ...
  iconContainer.connect('button-release-event',
                        function(actor, event) { icon.click(event); });
...
function onTrayIconRemoved(o, icon) {
  ...
  // A warning like this will logged if we try to call `destroy()` on `icon`
  // here:
  // """
  // Object Shell.TrayIcon (0x56014a3370e0), has been already deallocated —
